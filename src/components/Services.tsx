...
// استنتاج نوع عنصر الخدمة من الـ JSON نفسه
type Service = (typeof services.services)[number];

const getIcon = (iconName: string): LucideIcon => {
  // نحول الـ namespace لأنواع قابلة للفهرسة
              ...
              return (
                ...
                >
                  <div className="text-center space-y-3 lg:space-y-4">
                    <div className="w-12 h-12 lg:w-16 lg:h-16 mx-auto bg-primary/20 rounded-full flex items-center justify-center group-hover:bg-primary/30 transition-all duration-300">
                      <IconComponent
                        size={24}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedService.tools.map((tool, index) => (
                    <span
                      key={index}
                      className="px-2 lg:px-3 py-1 bg-primary/20 text-primary rounded-full text-xs lg:text-sm border border-primary/30"
                    >
                      {tool}
                    </span>
                  ))}
